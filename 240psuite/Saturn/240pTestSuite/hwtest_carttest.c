...
void draw_carttest(video_screen_mode_t screenmode)
{
	char buf[256];
	//removing text
	ClearTextLayer();
	...
	DrawString("Cartridge test", 160 - _fw*strlen("Cartridge test")/2, 20, FONT_WHITE);

	sprintf(buf,"Press B to Exit");
	DrawString(buf, 160 - strlen(buf)*3, 200, FONT_WHITE);
}

void hwtest_carttest(video_screen_mode_t screenmode)
{
